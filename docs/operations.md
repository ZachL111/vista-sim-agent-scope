...
`vista-sim-agent-scope` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for agent workflows, centered on storage recovery, log and snapshot fixtures, and replay consistency checks.
...
The score is deterministic. The current threshold is 177. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 4.
